...
const PendingElectionDeletion = require('../models/PendingElectionDeletion');
...
router.get('/fetching', async (req, res) => {
    try {
        ...
        if (admin.role === 'Head Admin') {
            ...
            // Filter by creator for other admins
            // Ensure we strictly filter by the exact username
            elections = await Election.find({ createdBy: admin.username }).sort({ createdAt: -1 });
        }

        // Add Voter Stats to each election
        const electionsWithStats = await Promise.all(elections.map(async (election) => {
            ...
            if (voterData && voterData.voters) {
                ...
                voted = voterData.voters.filter(v => v.isVoted).filter(Boolean).length; // Check Boolean just in case
    ...
    try {
        ...
        if (election) {
            ...
            if (now >= new Date(election.startTime) && now <= new Date(election.endTime)) {
                return res.status(400).json({ message: 'Cannot delete an election that is currently running.' });
            }
            // Send emails to all voters in the selected lists
...
router.get('/one/:id', async (req, res) => {
    try {
        ...
        let hasVoted = false;
        if (req.query.voterId && voterData && voterData.voters) {
            const v = voterData.voters.find(v => v.voterId === req.query.voterId);
            if (v && v.isVoted) hasVoted = true;
        }
        ...
        console.error("Error fetching single election:", error);
...
router.put('/one/:id', async (req, res) => {
    ...
    try {
        ...
        if (new Date() > new Date(currentElection.endTime)) {
            return res.status(400).json({ success: false, message: 'Cannot modify an election that has finished.' });
        }
...
router.put('/approve/:id', async (req, res) => {
    try {
        ...
        // --- Initialize Election Candidates (Vote Counts) ---
        const candidateListNames = approvedElection.candidateLists || [];
        const candidateListsDoc = await Candidate.find({ listname: { $in: candidateListNames } });

        const uniqueCandidatesMap = new Map();
        candidateListsDoc.forEach(list => {
            list.candidates.forEach(c => uniqueCandidatesMap.set(c.candidateId, c));
        });

        // Add System NOTA Candidate
        uniqueCandidatesMap.set('C1NOTA2', { candidateId: 'C1NOTA2' });

        const initialCandidates = Array.from(uniqueCandidatesMap.values()).map(c => ({
            candidateId: c.candidateId,
            voteCount: encryptVoteCount(0)
        }));

        await new ElectionCandidates({
            electionId: approvedElection._id,
            candidates: initialCandidates
        }).save();

        // --- Initialize Election Voters (Participation Status) ---
        const lists = await Voter.find({ listname: { $in: approvedElection.voterLists } });

        // Unique voters map to avoid duplicates if voter is in multiple lists
        const uniqueVotersMap = new Map();
        lists.forEach(list => {
            list.voters.forEach(v => uniqueVotersMap.set(v.voterId, v));
        });
        const allVoters = Array.from(uniqueVotersMap.values());

        const initialVoters = allVoters.map(v => ({
            voterId: v.voterId,
            isVoted: false
        }));
...
// POST /request-delete - Request deletion of an election
router.post('/request-delete', async (req, res) => {
    const { electionId, electionName, reason, requestedBy } = req.body;
    try {
        const election = await Election.findById(electionId);
        const now = new Date();
        if (election && now >= new Date(election.startTime) && now <= new Date(election.endTime)) {
            return res.status(400).json({ success: false, message: 'Cannot delete an election that is currently running.' });
        }

        const existingRequest = await PendingElectionDeletion.findOne({ electionId, status: 'Pending' });
        if (existingRequest) {
            return res.status(400).json({ success: false, message: 'Deletion request already pending for this election.' });
        }

        const newRequest = new PendingElectionDeletion({
            electionId,
            electionName,
            reason,
            requestedBy
        ...
        res.status(201).json({ success: true, message: 'Deletion request submitted.' });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Error requesting deletion', error: error.message });
    }
});

// GET /pending-deletions - Get all pending deletion requests
router.get('/pending-deletions', async (req, res) => {
    try {
        const requests = await PendingElectionDeletion.find({ status: 'Pending' });
        res.status(200).json({ success: true, requests });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Error fetching pending deletions', error: error.message });
    }
});

// POST /approve-delete/:id - Approve deletion
router.post('/approve-delete/:id', async (req, res) => {
    try {
        const request = await PendingElectionDeletion.findById(req.params.id);
        if (!request) return res.status(404).json({ success: false, message: 'Request not found' });

        // Reuse existing delete logic?
        // Current delete logic is at DELETE /trash with query parameter id.
        // We can call the same logic or duplicate it. Duplicating is safer to avoid route chaining complexity.

        const electionId = request.electionId;
        const election = await Election.findById(electionId);
        ...
        await PendingElectionDeletion.findByIdAndDelete(req.params.id); // Or set to Approved

        res.status(200).json({ success: true, message: 'Election deleted and request approved.' });
    } catch (error) {
        console.error('Error approving deletion:', error);
        res.status(500).json({ success: false, message: 'Error approving deletion', error: error.message });
    }
});

// POST /reject-delete/:id - Reject deletion
router.post('/reject-delete/:id', async (req, res) => {
    try {
        await PendingElectionDeletion.findByIdAndDelete(req.params.id); // Or set to Rejected status for history
        // If keeping history, use findByIdAndUpdate. But simplistic approach for now is delete.
        res.status(200).json({ success: true, message: 'Deletion request rejected.' });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Error rejecting deletion', error: error.message });
    }
});
...
router.post("/voters/create", async (req, res) => {
    ...
    try {
        ...
        if (admin.role === 'Head Admin') {
            ...
            const shortName = listname.replace(/[^a-zA-Z0-9]/g, '').substring(0, 4).toUpperCase() || 'VOTE';

            const votersWithPasswords = voters.map((voter, index) => ({
                ...voter,
                voterId: voter.voterId || `${shortName}${index + 1}`,
...
router.post("/candidates/create", async (req, res) => {
    ...
    try {
        ...
        if (admin.role === 'Head Admin') {
            // Head Admin can create directly
            const shortName = listname.replace(/[^a-zA-Z0-9]/g, '').substring(0, 4).toUpperCase() || 'CAND';

            const candidatesWithIds = candidates.map((candidate, index) => ({
                ...candidate,
                candidateId: candidate.candidateId || `${shortName}${index + 1}`
            }));
    ...
    try {
        ...
        res.status(200).json({
            ...
            candidates: uniqueCandidates.map(candidate => ({
                ...
                profile: candidate.profile, // Profile image URL
            })),
        });
    } catch (error) {
...
router.post('/cast-vote', async (req, res) => {
    ...
    try {
        ...
        try {
            const fullElection = await Election.findById(electionId);
            if (fullElection) {
                const lists = await Voter.find({ listname: { $in: fullElection.voterLists } });
                let targetVoter = null;

                for (const list of lists) {
                    const v = list.voters.find(vt => vt.voterId === voterId);
                    if (v) {
                        targetVoter = v;
                        break;
                    }
                ...
                if (targetVoter && targetVoter.email) {
                    sendVoteConfirmationEmail(targetVoter.email, targetVoter.voterName, fullElection.electionName)
                        .catch(err => console.error(`Failed to send confirmation email to ${targetVoter.email}:`, err));
                }
            }
        } catch (emailError) {
        ...
        // --- Auto-End Election Check ---
        try {
            const votersDoc = await ElectionVoters.findOne({ electionId });
            const totalVoters = votersDoc.voters.length;
            const votedCount = votersDoc.voters.filter(v => v.isVoted).length;

            if (totalVoters > 0 && votedCount === totalVoters) {
                console.log(`Election ${electionId} reached 100% turnout. Ending election...`);

                // 1. Mark Election as Ended
                const endedElection = await Election.findByIdAndUpdate(electionId, { endTime: new Date() }, { new: true });

                // 2. Determine Winner
                const candidateListNames = endedElection.candidateLists || [];
                const candidateListsDoc = await Candidate.find({ listname: { $in: candidateListNames } });

                // Create a map of candidateId -> candidateDetails
                const allCandidatesMap = new Map();
                candidateListsDoc.forEach(list => {
                    list.candidates.forEach(c => allCandidatesMap.set(c.candidateId, c));
                });

                // Also handle special candidates (like manually added NOTA if any, though system NOTA has ID 'C1NOTA2')
                // Note: The /electionCandidates-details endpoint hardcodes NOTA. 
                // We should check if 'C1NOTA2' is in the map, if not add it manually if needed, 
                // but usually NOTA is standard. 
                if (!allCandidatesMap.has('C1NOTA2')) {
                    allCandidatesMap.set('C1NOTA2', { candidateName: 'None of the Above', party: 'NOTA' });
                }

                const ec = await ElectionCandidates.findOne({ electionId });
                let maxVotes = -1;
                let topCandidates = [];
                ...
                if (ec && ec.candidates) {
                    ...
                    // Second pass: Collect all candidates with max votes
                    ec.candidates.forEach(c => {
                        const votes = decryptVoteCount(c.voteCount);
                        if (votes === maxVotes) {
                            // Get details from map
                            const details = allCandidatesMap.get(c.candidateId) || { candidateName: 'Unknown', party: 'Unknown' };
                            // Ensure candidateName is present (Candidate model uses candidateName, NOTA uses details provided)
                            const name = details.candidateName || details.name || 'Unknown Candidate';
                            topCandidates.push({ ...details, candidateName: name, candidateId: c.candidateId, votes });
                        }
                    });
                }
                ...
                if (topCandidates.length > 0) {
                    if (topCandidates.length > 1) {
                        ...
                        console.log(`Tie detected between: ${topCandidates.map(c => c.candidateName).join(', ')}. Randomly selected: ${winner.candidateName}`);
                    } else {
                        winner = topCandidates[0];
                    }
                }

                const winnerName = winner ? (winner.candidateName || winner.name || 'Unknown') : 'No Votes Cast';

                // Save Winner and Tie Status to Election (so it persists)
                ...
                votersDoc.voters.forEach(v => {
                    if (v.email) {
                        sendElectionEndEmail(v.email, v.voterName, endedElection.electionName, winnerName, isTie)
                            .catch(err => console.error(`Failed to send end email to ${v.email}:`, err));
                    }
                });
            }
        } catch (autoEndError) {
            console.error('Error during auto-end check:', autoEndError);
            // Don't fail the vote cast if auto-end fails
        }
        // --------------------------------
...
router.post('/approve-voter-list/:requestId', async (req, res) => {
    ...
    try {
        ...
        // Generate random passwords (and IDs if missing) for all voters
        const shortName = pendingRequest.listname.replace(/[^a-zA-Z0-9]/g, '').substring(0, 4).toUpperCase() || 'VOTE';

        const votersWithPasswords = pendingRequest.voters.map((voter, index) => ({
            ...voter,
            voterId: voter.voterId || `${shortName}${index + 1}`,
            password: generateRandomPassword()
        }));
...
router.post('/approve-candidate-list/:requestId', async (req, res) => {
    ...
    try {
        ...
        const shortName = pendingRequest.listname.replace(/[^a-zA-Z0-9]/g, '').substring(0, 4).toUpperCase() || 'CAND';

        const candidatesWithIds = pendingRequest.candidates.map((candidate, index) => ({
            ...candidate,
            candidateId: candidate.candidateId || `${shortName}${index + 1}`
        }));
