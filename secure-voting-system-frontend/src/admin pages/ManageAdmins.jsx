...
const ManageAdmins = () => {
  ...
  const handleDelete = async (id, username) => {
    const reason = prompt(
      `Enter reason for revoking access for ${username}.\nClick Cancel to abort.`
    );

    // Proceed if user didn't click Cancel. trim logic prevents empty spaces if desired, but optional.
    if (reason !== null) {
      if (!reason.trim()) {
        showMessage("Reason is required to revoke access.", "error");
        return;
      }
      try {
        await axios.delete(`https://secure-voting-system-second.onrender.com/api/admins/${id}`, { data: { reason } });
              ...
              <div key={admin.adminId} className="bg-slate-800 border border-slate-700 rounded-xl p-6 hover:border-slate-600 transition-all group">
                ...
                {admin.role !== 'Head Admin' && currentUser.role === 'Head Admin' && (
                  <button
                    ...
                    className="w-full py-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 rounded-lg text-sm font-medium transition-colors mt-2"
                  >
                    Remove Access
                  </button>
                )}
